    ...
    while EndFlag == 0:
        ...
        if Phase == 1:  # Wait for starting of task
            if Init == 0:   # If the task initialization has not been done
                ...
                Str = 'Go/NoGo task (' + str(Task) + ')    Start time ' + str(TaskStartMonth) + '/' + str(TaskStartDay) + ' ' + str(TaskStartHour) + ':' + str(TaskStartMinute) + '    Waiting...'  # Show status text in the main window
                Init = 1    # 1 means initialization has been done

        if Phase == 2:  # During task
            if Init == 0:  # Initialization of the task
                PutEndTaskNowButton()   # Put "task end" button
                Str = 'Go/NoGo task (' + str(Task) + ')    Start time ' + str(TaskStartMonth) + '/' + str(TaskStartDay) + ' ' + str(TaskStartHour) + ':' + str(TaskStartMinute) + '    Running'  # Show status text in the main window
                mSpace = ttk.Label(MainWindowRightFrame, text='    ').grid(row=0, column=1)  # Used just as spacer (GUI)
                mStatus = ttk.Label(MainWindowRightFrame, text=Str).grid(row=0, column=2, sticky=E)
                ...
                Writer_TouchEventTxt = open(    # Ready for text exporting of the result
                ...
                Writer_TouchEventCsv = open(    # Ready for csv exporting of the result
            ...
            if Phase2 == 1:  # Panel presentation
                ...
                if PanelType == 'Blink':    # If panel type is "blink"
                    if TimeNow.microsecond // 100 % 2 == 0: # Turn off or turn on every 100ms
                        HoleCueTurnOff(0)
                    if TimeNow.microsecond // 100 % 2 == 1: # Turn off or turn on every 100ms
                        HoleCueTurnOn(0)
                TouchSymbolCnt = 30
                if (TaskType == 'Go' and TouchedPanelID != -1) or (TaskType == 'NoGo' and TimerSec[0] >= PresentDur):  # If mouse responses correctly
                    ...
                    if TaskType == 'Go':    # If the task type is "GO"
                        Writer_TouchEventTxt.write(
                            str(TrialNum) + '\tPanelTouched(Correct)\t' + str(Year) + "/" + str(Month) + "/" + str(Day) + "\t" + str(Hour) + ":" + str(Minute) + "\t" + str(TimeNow.second) + "." + str(    # Write mice response information for the export
                                TimeNow.microsecond) + "\n")  # Write the response on the text file
                    ...
                    if TaskType == 'NoGo':    # If the task type is "NOGO"
                        Writer_TouchEventTxt.write(
                            str(TrialNum) + '\tPanelIgnored(Correct)\t' + str(Year) + "/" + str(Month) + "/" + str(Day) + "\t" + str(Hour) + ":" + str(Minute) + "\t" + str(TimeNow.second) + "." + str(    # Write mice response information for the export
                                TimeNow.microsecond) + "\n")  # Write the response on the text file
                        Writer_TouchEventTxt.write(
                            str(TrialNum) + ',PanelIgnored(Correct),' + str(Year) + "," + str(Month) + "," + str(Day) + "," + str(Hour) + "," + str(Minute) + "," + str(TimeNow.second) + "," + str(    # Write mice response information for the export
                                TimeNow.microsecond))  # Write the response on the csv file
                ...
                if (TaskType == 'NoGo' and TouchedPanelID != -1) or (TaskType == 'Go' and TimerSec[0] >= PresentDur):  # If mouse responses correctly
                    NowPunishing = 1
                    ...
                    if TaskType == 'Go':    # If the task type is "GO"
                        Writer_TouchEventTxt.write(
                            str(TrialNum) + '\tPanelIgnored(Wrong)\t' + str(Year) + "/" + str(Month) + "/" + str(Day) + "\t" + str(Hour) + ":" + str(Minute) + "\t" + str(TimeNow.second) + "." + str(    # Write mice response information for the export
                                TimeNow.microsecond) + "\n")  # Write the response on the text file
                        Writer_TouchEventCsv.write(
                            str(TrialNum) + ',PanelIgnored(Wrong),' + str(Year) + "," + str(Month) + "," + str(Day) + "," + str(Hour) + "," + str(Minute) + "," + str(TimeNow.second) + "," + str(    # Write mice response information for the export
                                TimeNow.microsecond))  # Write the response on the csv file
                    if TaskType == 'NoGo':    # If the task type is "NOGO"
                        Writer_TouchEventTxt.write(
                            str(TrialNum) + '\tPanelTouched(Wrong)\t' + str(Year) + "/" + str(Month) + "/" + str(Day) + "\t" + str(Hour) + ":" + str(Minute) + "\t" + str(TimeNow.second) + "." + str(    # Write mice response information for the export
            ...
            if Phase2 == -1:  # Task finish phase
                ...
                if CorrectRate >= NextTaskTh:   # If the correct rate exceed the threshold, change the task from next (This is used only in the housing analysis)
                    Task = NextTask
                SendEmail() # Send email to inform finishing of the task
                TtlDelayStop()  # Stop TTL sending

                Phase2 = 0
                if IsHousingAnalysis == 1:  # If this is the housing analysis
                    Phase = 1  # Go back to the task-waiting phase

                if IsConventionalAnalysis == 1: # If this is non-housing analysis
...
def OperantHouseUpdate():
    ...
    if SerialOutPhase==0:   # Select channel
        if CurrentChannelID==0: # If the selected channel No is 0
    ...
    if SerialOutPhase==1:    # Send value to regulate switching of the channel
        if CurrentChannelID==0: # If the selected channel No is 0
            TempBinary = bytes(str(TtlSquareOn), 'utf-8')   # Transform the status of TTL signal for synchronization (0:off 1:on) from integer to byte for sending to arduino through serial connection
        if CurrentChannelID==1:
        ...
        SerialOutPhase = 0
        CurrentChannelID += 1   # Change the selected channel No
        if CurrentChannelID >= 8:   # (because channel No is 0 to 7)
            CurrentChannelID = 0
